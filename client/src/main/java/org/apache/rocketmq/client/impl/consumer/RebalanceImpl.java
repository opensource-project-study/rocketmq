    ...
    public void lockAll() {
        ...
        while (it.hasNext()) {
            ...
            if (findBrokerResult != null) {
                ...
                try {
                    // 使用consumerGroup、clientId在broker端进行加锁，获取加锁成功的MessageQueue
                    Set<MessageQueue> lockOKMQSet =
                        this.mQClientFactory.getMQClientAPIImpl().lockBatchMQ(findBrokerResult.getBrokerAddr(), requestBody, 1000);

                    // 更新对应的ProcessQueue的锁定状态为加锁成功
                    ...
                    for (MessageQueue mq : mqs) {
                        // 处理加锁失败的MessageQueue对应的ProcessQueue
    ...
    private boolean updateProcessQueueTableInRebalance(final String topic, final Set<MessageQueue> mqSet,
        ...
        while (it.hasNext()) {
            ...
            if (mq.getTopic().equals(topic)) {
                // 如果mqSet中没有该mq，从processQueueTable中把mq->pq这个Entry移除
                if (!mqSet.contains(mq)) {
                    ...
                    switch (this.consumeType()) {
                        ...
                        case CONSUME_PASSIVELY:
                            // 如果mqSet中有该mq，但是pq过期了，也从processQueueTable中把mq->pq这个Entry移除
                            pq.setDropped(true);
                            if (this.removeUnnecessaryMessageQueue(mq, pq)) {
                                it.remove();
                                changed = true;
