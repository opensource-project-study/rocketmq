 ...
 * @date 2022/8/16
 */
public class PushConsumerNotRetry {

    private static final Logger LOGGER = LoggerFactory.getLogger(PushConsumerNotRetry.class);

    private static final int MAX_RECONSUME_TIMES = 20;

    public static void main(String[] args) throws InterruptedException, MQClientException {
        DefaultMQPushConsumer consumer = new DefaultMQPushConsumer("simple_message_consumer_group_2");
        consumer.setNamesrvAddr("localhost:9876");
        consumer.subscribe("TopicTest", "*");
        // 广播消费，不会消息重试；只有集群消费，才会消息重试。
